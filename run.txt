...
class crack:
	...
	def crack2(self,user,lala,asu):
		global ok,cp,cot,ajg
		ua=awokawok(["Mozilla/5.0 (Linux; Android 7.0; M6T) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/93.0.4577.82 Mobile Safari/537.36;]",open("ua","r").read()])
		print(f'\r[ CRACK ] {str(cot)}/{str(len(id))} | OK/CP:-{str(ok)}/{str(cp)} | {time.strftime("%X")}',end='')
		if ajg!=user:
			ajg=user
			cot+=1
	...
	def kirim(self):
		...
		if(pil in ("y","Y")):
			with Bool(max_workers=35) as kirim:
				print("[!] Contoh (sayang,bismillah,name name,name123,name 123)")
...
░░░░▄▄███▄▄░░░░░█████░
 ...
 By : Alpin4YOU
 Github : git clone https://github.com/AlpinXD/AlpinSempak
 
        
            
        """)
		self.jalan(f"[!] Welcome {self.nama}\n")
		...
		if(pil in ('01','1')):
			...
			os.system("xdg-open https://wa.me/6283119356487")
